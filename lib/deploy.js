...
function ftpUpload(output, md5, release, content, file, callback){
    var conf = fis.config.get("deploy.ftp.server");
    if(!conf){
        fis.log.error("ftp的配置不存在！");
        return;
    }
    var Ftp = require("./ftp");
    var ftp = Ftp.init(conf);
    ftp.put(file.realpath, output+"/"+file.basename, function(hadError) {
      if (!hadError){
        fis.log.notice(file.release+"文件上传成功!");
        callback();
    }else{
        fis.log.error(hadError);
    }
    })
...
function deploy(dest, file, callback){
    if(file.release){
        ...
        if(dest.receiver){
            ...
            if(dest.method === "ftp"){
                var to = dest.to.replace(dest.root,"");
                ftpUpload(to, dest.opt.md5, release, content, file, callback);
            }else{
    ...
    fis.util.map(collection, function(subpath, file){
        ...
        dests.forEach(function(d){
            if(
                ...
                //fis.log.notice(file.release+"即将被上传");
